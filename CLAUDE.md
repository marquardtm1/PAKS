...
- **Eingabe:** ~~schwebendes Text-Eingabefeld im Bildraum~~ → **abgelöst durch #31** (Texteingabe komplett in der „Markierungen"-Liste). Commit weiterhin gebündelt bei **Enter/Blur** (eine `updateCase`-Mutation → Undo bündelt), **Esc verwirft**.
...
- **Liste → Bild:** Klick auf eine „Markierungen"-Zeile wählt die Annotation im Bild aus. Ergänzt die bestehende Bild→Liste-Zuordnung über den Index. (Verfeinert in **#31**: Form-Symbol = nur Hervorheben, Text = inline editieren.)

### 31. Annotations-Texteingabe komplett in der Markierungsliste — ✅ ERLEDIGT
Alle Label-Eingabe/-Bearbeitung von #27/#28 vom Bild in die „Markierungen"-Liste verlagert — das schwebende Bild-Eingabefeld verdeckte den Befund.
- **Entfernt:** das Screen-Space-`<input>` im Bildraum samt `labelPos`-State, dem `useLayoutEffect` (imgRect-Messung + resize) und dem `requestAnimationFrame`-Fokus; `annotationAnchor`-Nutzung in `Lightbox` raus (bleibt für die Bild-Badges).
- **Zeile, zwei Klick-Ziele:** **Form-Symbol** → `highlightFromList` (nur Halo im Bild, kein Eingabefeld; aktiviert ggf. den Zeichen-Modus). **Text** → `startEdit` (Inline-`<input>` an Ort und Stelle).
- **Neuzeichnen:** `addAnnotation` → Form + auswählen + `startEdit` → die neue Zeile erscheint sofort mit `autoFocus` (direkt lostippen). Leer gelassen → `label: undefined` → Zeile verschwindet, Form bleibt unbeschriftet.
- **Bild-Form-Klick:** nur Hervorheben (`setSelectedAnnIds`) — kein Editor, Entf löscht weiter (kein Fokus-Klau).
- **Relabel-Lücke geschlossen:** die im Bild einzeln ausgewählte Form erscheint zusätzlich als Zeile (`listAnnotations = hasLabel || editingAnnId || singleSelected`) mit kursivem Platzhalter „Beschriftung …" → Text-Klick beschriftet sie nachträglich.
- **Commit:** Enter/Blur → eine `updateCase`-Mutation (undo-fähig), Esc verwirft; der Inline-Input kapselt seine Tasten (`stopPropagation`), damit Pfeile/Entf/Esc nicht die Lightbox steuern.
